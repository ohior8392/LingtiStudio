...
- **可审核**：高成本生成前先看脚本和分镜
- **可先确认资产**：先把人物、场景和道具图确认下来，再进入全量生成
...
1. 从主题或对标分析生成脚本
2. 在网页里审核并修改分镜
3. 先生成一组可确认的资产包：人物、场景风格、道具
4. 在网页里确认这些资产，再继续后续生成
5. 为每个分镜生成关键帧，并优先复用已确认资产保证一致性
6. 为每个分镜生成配音
7. 用 MiniMax Video、Kling 或 Seedance 生成视频片段
8. 自动组装转场、音频、字幕与最终成片
9. 导出 MP4、字幕文件、剪映 / CapCut 草稿
